...
def sli_importance(G, **kwargs):
    ...
    if kwargs.get('float') == 32:
        SLI_importance = np.array(SLI_importance).astype(np.float32)
    else:
        SLI_importance = np.array(SLI_importance)
    ...
    # SLI values normalized as default
        ...
        SLI_importance_normalized = (SLI_importance/np.sum(SLI_importance))

    ...
    for partition in partitions:
        try:
            ...
            commsDataFramePart=pd.DataFrame({'label': G.vs[partition]['label'],
                                             ...
                                             'Nsbb_importance': G.vs[partition]['sbb2_importance'], #dodano
                                             'NWsbb_importance': G.vs[partition]['sbb3_importance'], #dodano
...
def sbb2_importance(G):
    ...
    for node in nx.nodes(g):
        print(node, G.vs["label"][node], grp.neighborhood(g, node))
        # if node == veza[0]:
        suma = 0
...
def sbb3_importance(G):
    ...
    for node in nx.nodes(g):
        ...
        for susjed in grp.neighborhood(g, node):
            ...
            # print(node, g.degree(node), susjed, g.degree(susjed), dict1[(node, susjed)])
            #edge = (susjed, node)
            #for susjed, node, data in G.edges(data=True):
            #    print(data["weight"])
            u = (G.vs[node]["weighted_degree"]+dict1[(node, susjed)] + G.vs[susjed]["weighted_degree"])#*G.edge[edge[susjed]][[node]]["weight"]
            w = (dict1[(node, susjed)]/2+1)

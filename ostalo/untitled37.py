...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  5 13:46:28 2024
...
def calculate_importance(G, edge_cycle_count, node_strength, node_cycles):
    ...
    for node in G.nodes():
        ...
        for neighbor in G.neighbors(node):
            ...
            cycle_count = edge_cycle_count.get(edge, 0)
    ...
    # Correctly calculate the total sum of importance scores
    total = sum(importance_scores.values())

    # Ensure total is a numerical value and greater than 0 to avoid division by zero
    if total > 0:  # This comparison should now work as expected
        normalized_scores = {node: (score / total * 100) for node, score in importance_scores.items()}
    else:
        normalized_scores = {node: 0 for node in importance_scores.keys()}  # Provide a default score if total is 0

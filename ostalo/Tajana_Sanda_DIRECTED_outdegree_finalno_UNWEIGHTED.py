...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 25 23:26:29 2024
...
print("Pocinje petlja koja nam treba za + i unweighted")
...
for node in G.nodes:
    ...
    for x in list(G.predecessors(node)):   
        print("Susjed, vrh: ", x, ",",  node)
            #print("dict counter veze", dict[(node, x)])
            #dict2['key'] if 'key' in dict2 else 0
        zagrada = zagradica(node) + zagradica(x) - G.get_edge_data(x, node)['weight']
        print("zagrada", zagrada)
...
for key in dict2:
    if key in dict3:
        my_dict[key] = dict4[key] + dict3[key]
    else:
        pass

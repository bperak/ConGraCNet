...
# for par in G.degree(weight='weight'):
#     node_Weight= par[1]
#     print(node_Weight)
...
def sbb3_importance(G):
    ...
    for node in nx.nodes(G):
        ...
        for susjed in gp.neighborhood(G, node):
            ...
            edge_weight= G.get_edge_data(node,susjed)['weight']
            print('edge_weight', node, susjed, edge_weight)
            
            nodeWeight_node = G.nodes[node]["weighted_degree"]
            print('nodeWeight_node', node, nodeWeight_node)
            
            nodeWeight_susjed =  G.nodes[susjed]["weighted_degree"]
            print('nodeWeight_susjed', susjed, nodeWeight_susjed)
            ...
            # edge_weight= G.get_edge_data(node,susjed)['weight']
            # nodeWeight_node = G.degree(node)['weight']

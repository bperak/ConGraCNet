...
def zagradica(node, in_strength, out_strength):
...
def calculate_importance(G, edge_cycle_count, in_strength, out_strength):
    ...
    for node in G.nodes():
        ...
        for x in list(G.successors(node)) + list(G.predecessors(node)):
            ...
            zagrada = zagradica(node, in_strength, out_strength) + zagradica(x, in_strength, out_strength) - 2 * G.get_edge_data(*edge)['weight']
            w = G.get_edge_data(*edge)['weight']
            omjer = zagradica(node, in_strength, out_strength) / (zagradica(node, in_strength, out_strength) + zagradica(x, in_strength, out_strength))
            partial_umnozak = (cycle_count + 1) * zagrada * w * omjer
            umnozak += partial_umnozak

        # Now, calculate the degrees sum and adjust the total umnozak
        strength_sum = in_strength[node] + out_strength[node]
        adjusted_umnozak = umnozak + strength_sum
        ...
        print(f"Total umnozak for Node {node}: {umnozak}")

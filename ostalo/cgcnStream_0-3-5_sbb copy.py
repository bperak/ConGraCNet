...
with st.beta_expander('Clustered 2nd degree network with '+str(len(df_fof))+' nodes and '+str(len(clusterAlgo))+\
...
sbb_ntile_prun=st.sidebar.slider('Sbb importance percentile', 0.1,100.0,50.0)
ntile_sbb = np.percentile(btwn, sbb_ntile_prun)
pruned_vs = FoFgraph.vs.select([v for v, b in enumerate(btwn) if b >= ntile_sbb])
pruned_graph = FoFgraph.subgraph(pruned_vs)
...
sbb2_ntile_prun=st.sidebar.slider('Sbb2 importance percentile', 0.1,100.0,50.0)
ntile_sbb2 = np.percentile(btwn, sbb2_ntile_prun)
pruned_vs = FoFgraph.vs.select([v for v, b in enumerate(btwn) if b >= ntile_sbb2])
pruned_graph = FoFgraph.subgraph(pruned_vs)

# prune by shishi_importance2
pruned_graph.vs['sbb3_importance']= cgcn_functions.sbb3_importance(pruned_graph)
sbb3=pruned_graph.vs['sbb3_importance']
sbb3_ntile_prun=st.sidebar.slider('Sbb3 importance percentile', 0.1,100.0,50.0)
ntile_sbb3 = np.percentile(btwn, sbb3_ntile_prun)
pruned_vs = FoFgraph.vs.select([v for v, b in enumerate(btwn) if b >= ntile_sbb3])
pruned_graph = FoFgraph.subgraph(pruned_vs)

# prune by shishi_importance2
pruned_graph.vs['shishi_importance']= cgcn_functions.shishi_importance(pruned_graph)
shishi=pruned_graph.vs['shishi_importance']
shishi_ntile_prun=st.sidebar.slider('Shi - Shi importance percentile', 0.1,100.0,50.0)
ntile_shishi = np.percentile(btwn, shishi_ntile_prun)
pruned_vs = FoFgraph.vs.select([v for v, b in enumerate(btwn) if b >= ntile_shishi])
pruned_graph = FoFgraph.subgraph(pruned_vs)

# prune by degree
pruned_graph.vs['degree']= pruned_graph.degree()
maxDegree= pruned_graph.maxdegree(vertices=None, loops=False)
prunedDegree = st.sidebar.slider('Degree pruned', 0, maxDegree, (1,maxDegree))

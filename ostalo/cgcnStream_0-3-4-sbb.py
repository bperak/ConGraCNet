...
# sanda test
if st.checkbox('Sanda test'):
    G=FoFgraph
    #st.write(G)
    #graph_12=G.get_edgelist()
    #G.vs["name"] = G.get_edgelist()
    g=G.to_networkx()
    st.write(g)
    #g = nx.parse_gml("test123")
    #st.write(g)
    #V(g)$name

    ciklus = nx.cycle_basis(g.to_undirected())

# Will contain:
# {
#   node_value => sum_of_degrees
# }
    degrees = {}
    a=[]
    ...
    #a.append(list(permutations(veza)))
    #print(a)
        ...
        #print("set2: ", set2)
            ...
            if is_subset==True:
                counter_veze+=1
            #print("Veza: ", set1, " counter veze= ", counter_veze)
    #print ("Finalno: Veza:", veza, " counter veze= ", counter_veze)
    ...
    #vaznost_vrha=g.degree(veza[0])+
    #print("Vrh", veza[0], "ima vaznost", vaznost_vrha)

    #st.write("Ispis nodova i pripadajucih stupnjeva:")
    #st.write(degrees)

    #st.write("Stupnjevi po vrhovima", g.degree())

#print("Suma svih stupnjeva u grafu je", sum(degree.get(g)[1]))
    zbroj_stupnjeva=0
    ...
    for j in nx.nodes(g):   
        st.write("Vaznost cvora {} je: ".format(G.vs[j]["name"]), round((g.degree(j)+degrees.get(j))/zbroj_stupnjeva, 5), "a weight je", G.vs[j]["weighted_degree"])

    #st.write(zbroj_stupnjeva)

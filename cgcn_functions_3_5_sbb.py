...
@st.cache(allow_output_mutation=True)
def Fgraph_is_a(dataset, measure):
    'calculate a graph from dataset of source - friend coocurences with a chosen measure'
    #create df variable
    df=dataset[['source', 'friend', measure]]
    #create tuples from df.values
...
# def df_from_graph(G):
#     'create a dataframe from graph and sort lempos'
#     df= pd.DataFrame().from_dict(G.vs) #columns=['label', 'degree', 'pagerank', 'weighted_degree', 'betweenness', 'eigen_c'])
#     # try:
#     #     df['label']=G.vs['name']
#     #     df['degree']=G.vs['degree']
#     #     df['pagerank']=G.vs['pagerank']
    ...
    for partition in partitions:
        try:
            ...
            commsDataFramePart=pd.DataFrame({'label': G.vs[partition]['label'],
                                             ...
                                             'weighted': G.strength(G.vs[partition]["label"], weights='weight', mode='ALL'),
                                             ...
                                             'sbb3_importance': G.vs[partition]['sbb3_importance'],
                                             'sbb3_norm': G.vs[partition]['sbb3_norm'],
                                             'shishi_importance': G.vs[partition]['shishi_importance'],
...
#             u = (G.vs[node]["weighted_degree"]+dict1[(node, susjed)] + G.vs[susjed]["weighted_degree"])
#             w = (dict1[(node, susjed)]/2+1)
#             z = (G.vs[node]["weighted_degree"])/(G.vs[node]["weighted_degree"]+G.vs[susjed]["weighted_degree"])
#             rezultat0 = u*w
#             rezultat = u*w*z
#             # print('u, w, z, rezultat0, rezultat:', u, w, z, rezultat0, rezultat)
#             suma = suma + rezultat
#         # print(suma + G.vs[node]["weighted_degree"])
#         # print(G.vs["label"][node], G.vs[node]["weighted_degree"])
#         nasa_vaznost1.append(suma + G.vs[node]["weighted_degree"])
#         #nasa_vaznost1.append(suma*G.vs[node]["weighted_degree"] + g.degree(node))
...
def sbb2_importance(G):
    ...
    for node in nx.nodes(g):
        ...
        suma = 0
        for susjed in grp.neighborhood(g, node):
            # naći weight na edge  node-susjed
            ...
            # u = (G.vs[node]["weighted_degree"]+dict1[(node, susjed)] + G.vs[susjed]["weighted_degree"]-(2*edge_weight)])*edge_weight
            lam = (dict1[(node, susjed)]/2+1)
            # w = (dict1[(node, susjed)]/2+1)
            # z= 1
            z = edge_weight
            # z = (G.vs[node]["weighted_degree"])/(G.vs[node]["weighted_degree"]+G.vs[susjed]["weighted_degree"])
        ...
        # print(suma + G.vs[node]["weighted_degree"])
        # print(G.vs["label"][node], G.vs[node]["weighted_degree"])
        nasa_vaznost1.append(suma + nodeWeight_node)
        #nasa_vaznost1.append(suma*G.vs[node]["weighted_degree"] + g.degree(node))
            #china_vaznost.append((weightlast*G.vs[node]["weighted_degree"] + g.degree(node))) #za normalizaciju jos dodati /suma_vrijednosti

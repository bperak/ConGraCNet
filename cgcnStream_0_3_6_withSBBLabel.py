...
# Clean the DataFrame to handle NaN values before plotting
# Remove rows with NaN values in critical columns
df = df.dropna(subset=['centrality_sum', 'polarity_value'])

# Check if we have any data left after cleaning
if df.empty:
    st.warning("No valid data available for plotting after removing NaN values.")
    st.stop()

# Debug: Show data info before plotting
st.write(f"DataFrame shape after cleaning: {df.shape}")
...
# napravi vrijednosti srednjica zajednica gsv_comm
...
required_columns = ['comm', 'polarity_value', 'centrality_sum', 'WordNet_hypernym_label', 'postotak']
missing_columns = [col for col in required_columns if col not in df.columns]
if missing_columns:
    st.error(f"Missing required columns for plotting: {missing_columns}")
    st.stop()

# Create the scatter plot with additional safety checks
try:
    ...
    st.error("DataFrame info:")
    st.write(df.info())
    st.error("DataFrame head:")
    st.write(df.head())
    st.stop()

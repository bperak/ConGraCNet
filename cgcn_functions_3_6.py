...
def FgraphDraW_plotly(Fgraph, layout, vertexSize, vertexLabelSize, imageName, edgeLabelSize):   
    ...
    for e in E:
        fig.add_trace(go.Scatter(x=[layt[e[0]][0],layt[e[1]][0], None], y=[layt[e[0]][1],layt[e[1]][1], None],
                mode='lines', name='Edges',
                # line= dict(width=G.es[G.get_eid(e[0], e[1])]['weight'],
                line= dict(width= 1, 
                color='rgba(16, 112, 2, 0.1)', shape='spline', smoothing=1.3),
                hoverinfo='none'
                ))

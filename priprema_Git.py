...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 27 14:41:47 2024
...
def katz_bonacich_centrality(G, alpha=0.1, beta=1, tol=1e-6, max_iter=100):
    ...
    for _ in range(max_iter):
        ...
        katz_cent = {node: beta for node in G.nodes}

        for node in G.nodes:
            for neighbor in G.neighbors(node):
                katz_cent[node] += alpha * A[int(node)][int(neighbor)] * prev_katz_cent[neighbor]
...
katz_cent = katz_bonacich_centrality(G)

# Print centrality scores
print("Katz-Bonacich centrality scores:")
for node, centrality in katz_cent.items():
